...
import { requireEasyComeAdmin } from '../server/_demo-auth.js';
import { textSearch, placeDetails } from '../server/_google-places.js';
import { seenPlaceIds, createCampaign, insertTargets, recentCampaigns, campaignTargets, markCampaign } from '../server/_demo-store.js';
import { buildQueryPlan, classifyPlace, buildDemoModel, demoSlug, outreachMessage } from '../server/_demo-factory-core.js';
...
export default async function handler(req,res){
  try{
    ...
    const plan=buildQueryPlan(`${existing.size}:${campaignId}`,240); const selected=[]; const inBatch=new Set(); let queriesRun=0; let rawSeen=0;
    try{
      const concurrency=6;
      ...
      return res.status(200).json({campaign:{...campaign,status:selected.length===limit?'completed':'partial',generated_count:selected.length,queries_run:queriesRun},targets,stats:{requested:limit,generated:selected.length,alreadySeen:existing.size,queriesRun,rawSeen},warning:selected.length<limit?`Trovate ${selected.length} nuove attività prima del limite di sicurezza delle query. Premi di nuovo Genera: i Place ID già usati resteranno esclusi.`:''});
