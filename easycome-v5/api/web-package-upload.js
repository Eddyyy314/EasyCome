...
export default async function handler(req,res){
  try{
    ...
    if(mode==='finalize'){
      ...
      const origin=appOrigin(req),hostedPreviewUrl=`${origin}${portalBase}`,publishedAt=new Date().toISOString(),isFinalHandoff=Boolean(current.websiteHandoff?.stage==='final'&&current.clientBrief?.submittedAt&&current.websiteHandoff?.clientBriefSubmittedAt===current.clientBrief.submittedAt),next={...proposal,packagePath:objectPath,packageName:proposal.packageName||clean(objectPath.split('/').pop()),packageBytes:source.bytes.length,packageUploadedAt:publishedAt,uploadPending:false,status:proposal.status==='paid'?'paid':'ready',buildState:'ready',builderVersion:BUILDER_VERSION,previewMode:'easycome-hosted',previewUrl:hostedPreviewUrl,hostedPreviewUrl,siteBasePath,siteRoot:site.root,siteFileCount:site.files.length,siteBuildMode:site.buildMode,sitePublishedAt:publishedAt,finalizedAt:isFinalHandoff?publishedAt:(proposal.finalizedAt||null),needsRevision:isFinalHandoff?false:Boolean(proposal.needsRevision),buildDiagnostics:site.diagnostics||null,updatedAt:publishedAt};
