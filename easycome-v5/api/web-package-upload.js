...
export default async function handler(req,res){
  try{
    ...
    if(mode==='finalize'){
      ...
      const entries=zipEntries(source.bytes);businessUxGate(entries,current.websiteAiBrief?.imageManifest||[]);const portalBase=`/web-sites/${encodeURIComponent(slug)}/${encodeURIComponent(token)}/`,site=await resolveSite(entries,portalBase),siteBasePath=`projects/${slug}/${token}/site-v${Date.now()}`;
