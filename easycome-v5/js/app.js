...
function draftKey(){return `easycome-hospitality-v4:${activeUserId||'guest'}`}
...
function recommendedSet(p=project){
 ...
 if((h.priorities||[]).includes('tax'))r.add('tourist_tax');
 if((h.priorities||[]).includes('numbers')){r.add('finance');r.add('expenses')}
 if((h.priorities||[]).includes('pricing') || Number(h.unitCount)>=6 || h.bookingVolume==='high')r.add('dynamic_pricing');
 if(h.teamSize!=='solo')r.add('multiuser');
 if(h.automationLevel==='high'||h.bookingVolume==='high'||Number(h.unitCount)>=7)r.add('automations');
 if((h.priorities||[]).includes('payments') && h.bookingVolume==='high')r.add('automations');
 return r;
}
...
function recSidebar(){const list=recommendationFeatures().slice(0,5);return `<aside class="recommendation"><span class="eyebrow">IN BASE ALLE TUE RISPOSTE</span><h3>Ti consigliamo<br><em>${project.hospitality.preset==='base'?'Essenziale':project.hospitality.preset==='full'?'Completo':'una configurazione su misura'}.</em></h3><div class="recommendation-list">${list.map(f=>`<div class="rec-item"><b>${esc(f.title)}</b><span>${esc(f.desc)}</span></div>`).join('')||'<div class="rec-item"><b>Gestione essenziale</b><span>Le cinque aree principali sono già incluse.</span></div>'}</div><div class="rec-reason">Perché: ${esc(recommendationReason())}.</div><button type="button" class="btn-rec" id="goPlans">Vedi le configurazioni</button></aside>`}
function stepStructure(){const h=project.hospitality,c=project.company;return `${heading('CONFIGURAZIONE','Raccontaci la tua struttura.','Poche domande utili per capire come lavori e proporti una configurazione sensata. Nessun campo tecnico, nessuna impostazione da sviluppatore.')}
<div class="config-layout">
<aside class="config-index"><div class="intro"><span class="eyebrow">PASSO 1</span><h2>Capire prima.<br>Configurare dopo.</h2><p>Rispondi solo a ciò che conosci già del tuo lavoro quotidiano.</p></div><nav class="mini-nav"><button class="active"><b>1</b>La struttura</button><button><b>2</b>Prenotazioni</button><button><b>3</b>Come lavori</button><button><b>4</b>Cosa vuoi controllare</button><button><b>5</b>Come gestisci oggi</button></nav></aside>
<div class="config-form">
<section class="card"><div class="section-title"><div class="number-title"><span class="section-num">1</span><h2>La tua struttura</h2></div><p>La base per dimensionare Easy Come.</p></div><div class="types">${['B&B','Affittacamere','Casa vacanza'].map(v=>`<button type="button" class="type-btn ${h.type===v?'active':''}" data-type="${v}"><b>${v}</b><small>${typeCopy(v)}</small></button>`).join('')}</div><div class="fields"><label class="field"><span>Nome della struttura</span><input id="companyName" value="${esc(c.name||'')}" placeholder="Es. Dimora Aurora"></label><label class="field"><span>Città</span><input id="city" value="${esc(h.city||'')}" placeholder="Es. Roma"></label><label class="field"><span>Camere / alloggi</span><input id="unitCount" type="number" min="1" max="100" value="${Number(h.unitCount||4)}"></label></div></section>
<section class="card"><div class="section-title"><div class="number-title"><span class="section-num">2</span><h2>Come arrivano le prenotazioni?</h2></div><p>Seleziona tutti i canali che utilizzi.</p></div><div class="question-grid"><div class="question wide"><div class="question-head"><div><h3>Canali</h3><p>Serve a capire quanto è importante allineare calendari e disponibilità.</p></div></div><div class="choices">${choiceButtons(CHANNELS,'channel',h.channels,true)}</div></div><div class="question wide"><div class="question-head"><div><h3>Quante prenotazioni gestisci in un mese normale?</h3><p>Una stima va benissimo.</p></div></div><div class="choices">${choiceButtons([['low','Meno di 15'],['medium','15–50'],['high','Più di 50']],'bookings',h.bookingVolume)}</div></div></div></section>
<section class="card"><div class="section-title"><div class="number-title"><span class="section-num">3</span><h2>Come lavori ogni giorno?</h2></div><p>Così evitiamo funzioni inutili.</p></div><div class="question-grid"><div class="question"><div class="question-head"><div><h3>Team</h3><p>Quante persone lavorano con Easy Come?</p></div></div><div class="choices">${choiceButtons([['solo','Solo io'],['small','2–3 persone'],['team','4+ persone']],'team',h.teamSize)}</div></div><div class="question"><div class="question-head"><div><h3>Check-in</h3><p>Come accogli normalmente gli ospiti?</p></div></div><div class="choices">${choiceButtons([['presence','In presenza'],['self','Self check-in'],['mixed','Entrambi']],'checkin',h.checkinMode)}</div></div><div class="question wide"><div class="question-head"><div><h3>Pulizie</h3><p>Chi prepara camere o alloggi?</p></div></div><div class="choices">${choiceButtons([['internal','Le gestiamo noi'],['external','Impresa / esterni'],['mixed','Entrambi']],'cleaning',h.cleaningMode)}</div></div></div></section>
<section class="card"><div class="section-title"><div class="number-title"><span class="section-num">4</span><h2>Cosa vuoi tenere sotto controllo?</h2></div><p>Puoi scegliere più aree.</p></div><div class="choices">${PRIORITIES.map(([id,label])=>`<button type="button" data-priority="${id}" class="choice icon-choice ${(h.priorities||[]).includes(id)?'active':''}"><span>${label}</span></button>`).join('')}</div></section>
<section class="card"><div class="section-title"><div class="number-title"><span class="section-num">5</span><h2>Come gestisci oggi?</h2></div><p>Serve per capire quanto deve essere guidato il passaggio.</p></div><div class="question-grid"><div class="question"><div class="question-head"><div><h3>Strumento attuale</h3><p>Dove tieni oggi prenotazioni e dati?</p></div></div><div class="choices">${choiceButtons([['manual','Agenda / manuale'],['sheets','Excel / fogli'],['pms','Altro gestionale']],'tool',h.currentTool)}</div></div><div class="question"><div class="question-head"><div><h3>Automazione</h3><p>Quanto vuoi toglierti attività ripetitive?</p></div></div><div class="choices">${choiceButtons([['light','Solo promemoria'],['medium','Alcune attività'],['high','Il più possibile']],'automation',h.automationLevel)}</div></div></div></section>
</div>${recSidebar()}</div>`}

function packageFeatureLines(mode){
 if(mode==='base')return ['Gestione completa delle prenotazioni','Calendario e disponibilità','Operazioni quotidiane','Dati ospiti nel soggiorno'];
 if(mode==='full')return ['Tutto ciò che include il Consigliato','Costi, margini e tariffe','Automazioni e controlli avanzati','Team, canali e accoglienza completa'];
 const rec=recommendationFeatures();return ['Tutto ciò che include Essenziale',...rec.slice(0,3).map(f=>f.title)];
}
function packageIdeal(mode){const h=project.hospitality;if(mode==='base')return Number(h.unitCount)<=3?'Strutture piccole con un flusso semplice.':'Chi vuole partire dal nucleo essenziale.';if(mode==='full')return 'Chi vuole attivare tutte le capacità Hospitality disponibili.';return `Per come lavori oggi: ${recommendationReason()}.`}
function presetCard(mode,title,kicker,icon){const selected=project.hospitality.preset===mode,rec=mode==='recommended';return `<article class="preset-card ${selected?'selected':''} ${rec?'recommended':''}" data-plan-card="${mode}"><div class="preset-icon">${icon}</div><span class="eyebrow">${kicker}</span><h3>${title}</h3><p>${mode==='base'?'Tutto l’indispensabile per iniziare.':mode==='full'?'Tutte le capacità di Easy Come Hospitality.':'Il giusto equilibrio per la tua struttura.'}</p>${rec?`<div class="why-this">Per ${Number(project.hospitality.unitCount||1)} camere/alloggi: <b>${esc(recommendationReason())}</b>.</div>`:''}<div class="preset-features">${packageFeatureLines(mode).map(x=>`<span>${esc(x)}</span>`).join('')}</div><div class="ideal"><b>IDEALE PER</b><span>${esc(packageIdeal(mode))}</span></div><div class="preset-price"><div><strong>${money(presetPrice(mode))}</strong><small> una tantum · implementazione inclusa</small></div><button type="button" class="select-plan" data-preset="${mode}">${selected?'Scelto':`Scegli ${title}`}</button></div></article>`}
function stepNeeds(){return `${heading('PERSONALIZZA SENZA COMPLICARTI','Come vuoi partire?','Tre configurazioni chiare. Ti evidenziamo quella che ha più senso in base alle risposte che ci hai dato; puoi comunque cambiare tutto.')}
<section class="preset-stage"><div class="preset-top"><div><span class="eyebrow">SCELTA RAPIDA</span><h2>Non devi conoscere i moduli.</h2></div><p>Ogni configurazione contiene sempre il nucleo operativo. Cambia soltanto il livello di controllo, automazione e analisi.</p></div><div class="preset-grid">${presetCard('base','Essenziale','SEMPLICE','◇')}${presetCard('recommended','Consigliato','PER LA TUA STRUTTURA','★')}${presetCard('full','Completo','TUTTO ATTIVO','♛')}</div><div class="included-strip"><b>Sempre incluso</b><span>Oggi</span><span>Calendario</span><span>Prenotazioni</span><span>Operazioni</span><span>Numeri base</span><span>Dati ospiti</span></div></section>
<section class="customize"><button type="button" class="customize-toggle" id="customizeToggle"><div><b>Vuoi rifinire la scelta?</b><span>È opzionale. Apri solo se vuoi aggiungere o togliere singole capacità.</span></div><strong>${customizeOpen?'−':'+'}</strong></button>${customizeOpen?`<div class="feature-grid">${FEATURES.map((f,i)=>`<button type="button" class="feature ${featureActive(f)?'active':''}" data-feature="${i}"><div class="feature-top"><span>${f.tag}</span><i>${featureActive(f)?'Attivo':'Aggiungi'}</i></div><h3>${f.question}</h3><p>${f.desc}</p><footer><b>${f.title}</b><strong>+${money(featurePrice(f))}</strong></footer></button>`).join('')}</div>`:''}</section>`}
