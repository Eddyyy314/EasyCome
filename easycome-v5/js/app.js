(function () {
  ...
  function normalizeProject(value) {
    ...
    p.delivery.managedServiceSelected = true;
    p.delivery.implementationPrice = 150;
    p.delivery.managedServicePrice = 150;
  ...
  function deliveryStep() {
    ...
    return `<div class="panel-heading"><div><span class="eyebrow">Passaggio 7</span><h1>Hai costruito il tuo Easy Come.</h1><p>Qui non scegli servizi tecnici: sono parte del prodotto. Il software viene implementato da Easy Come e resta operativo con il servizio continuativo.</p></div><div class="heading-badge ${ready ? 'success' : ''}">${audit.score}/100 · ${esc(audit.grade)}</div></div>
      ...
      <section class="quote-card"><span class="eyebrow">Investimento trasparente</span><div class="quote-lines"><div><span>Pacchetto software</span><strong>${money(price.base)}</strong></div><div><span>Moduli scelti</span><strong>${money(price.modules)}</strong></div><div><span>Sezioni e campi su misura</span><strong>${money(price.customEntities + price.customFields)}</strong></div><div><span>Automazioni</span><strong>${money(price.automations)}</strong></div><div><span>Regole prezzo extra</span><strong>${money(price.pricingRules)}</strong></div>${price.bundleDiscount ? `<div><span>Sconto bundle funzioni</span><strong>- ${money(price.bundleDiscount)}</strong></div>` : ''}<div><span>Implementazione Easy Come · obbligatoria</span><strong>${money(price.implementation)}</strong></div></div>
        <div class="mandatory-service-card"><span>DOPO L’ATTIVAZIONE</span><strong>€150 <small>/ mese</small></strong><h3>Easy Come Operativo</h3><p>È ciò che mantiene il sistema realmente utilizzabile: gestione tecnica, continuità del software e del database configurato, manutenzione, aggiornamenti compatibili e assistenza Easy Come.</p><div><b>✓ Un unico riferimento tecnico</b><b>✓ Sistema e database seguiti</b><b>✓ Manutenzione e aggiornamenti</b><b>✓ Assistenza e continuità operativa</b></div></div>
        <div class="quote-total"><span>Da pagare all’attivazione</span><strong>${money(price.total)}</strong><small>Poi €150/mese per il funzionamento continuativo Easy Come.</small></div><label class="field"><span>Note commerciali</span><textarea id="deliveryNotes" placeholder="Esigenze particolari, migrazione dati, formazione…">${esc(project.delivery.notes)}</textarea></label><button id="printQuote" class="btn btn-secondary" style="width:100%">Stampa offerta professionale</button></section></div>
      <div class="truth-card"><strong>Niente opzioni tecniche nascoste.</strong><p>Il software ha un prezzo una tantum variabile in base a ciò che costruisci. L’implementazione costa €150 ed è obbligatoria. Easy Come Operativo costa €150/mese ed è necessario per mantenere il sistema in funzione e seguito.</p></div>`;
  }
  ...
  function openCheckout(preparedResult) {
    ...
    root.innerHTML = `<div class="checkout-overlay" role="dialog" aria-modal="true" aria-labelledby="checkoutTitle"><section class="checkout-sheet">
      <div class="checkout-copy"><div class="checkout-top"><div><span class="process-kicker">ORDINE EASY COME</span><h2 id="checkoutTitle">Il tuo sistema digitale è pronto.</h2><p>Controlla i dati e passa al pagamento sicuro.</p></div><button class="checkout-close" id="closeCheckout" type="button" aria-label="Chiudi">×</button></div>
        ...
        <div class="order-total"><div><span class="checkout-label">TOTALE DA PAGARE ORA</span><small>Poi Easy Come Operativo: 150 € al mese</small></div><strong>${money(price.total)}</strong></div><div class="managed-checkout-note"><b>↻</b><span><strong>Easy Come Operativo · obbligatorio</strong><small>150 € / mese · gestione tecnica del sistema configurato, manutenzione, aggiornamenti compatibili e assistenza Easy Come.</small></span></div>
        <div class="checkout-guarantees"><span>✓ Anteprima approvata prima del pagamento</span><span>✓ Implementazione Easy Come inclusa e obbligatoria</span><span>✓ Easy Come Operativo: 150 € / mese</span><span>✓ Pagamento protetto tramite Stripe</span><span>✓ Pacchetto disponibile dopo la conferma Stripe</span></div>
      </div>
      <div class="checkout-form-wrap"><form id="checkoutForm" class="checkout-form"><span class="checkout-label">DATI PER L’ORDINE</span><h3>A chi intestiamo il progetto?</h3><p>Questi dati vengono associati al pagamento e alla consegna del pacchetto software.</p>
        ...
        <div id="checkoutError"></div><button class="btn btn-primary checkout-button" id="payButton" type="submit">${`Paga ${money(price.total)} e attiva 150 € / mese`}</button><div class="secure-row"><span>🔒</span><span>I dati della carta vengono gestiti direttamente da Stripe.</span></div>
      </form></div>
    </section></div>`;
    ...
    $('#checkoutForm').onsubmit = async (event) => {
      ...
      try {
        ...
        if (!accessToken) {
          ...
          button.textContent = `Paga ${money(price.total)} e attiva 150 € / mese`;
          return;
        }
