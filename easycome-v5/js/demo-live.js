(function(){
  ...
  async function init(){
    try{
      ...
      $('#app').className='';
      $('#app').innerHTML=`<div class="demo-shell"><aside class="side"><div class="brand"><span class="brand-mark">EC</span><div><small>ANTEPRIMA CREATA PER</small><strong>${esc(place.name)}</strong></div></div><div class="demo-badge">DEMO INTERATTIVA</div><nav class="nav">${model.nav.map((x,i)=>`<button class="${i===activeIndex?'active':''}" data-index="${i}"><span>${sectionIcon(x)}</span><b>${esc(x)}</b></button>`).join('')}</nav><div class="side-info"><span>Questa è una simulazione</span><p>I dati operativi sono inventati. Il nome e le informazioni pubbliche dell’attività servono solo a rendere l’anteprima concreta.</p></div><div class="side-footer"><b>Easy Come</b><small>Più ordine. Meno caos.</small></div></aside><main class="main"><div class="demo-topline"><div><span class="pulse"></span> ${esc(expiry)}</div><div class="top-actions"><a href="${studio}" id="topCta">Adattalo al tuo lavoro</a></div></div><div id="view"></div><section class="cta"><div><span>QUESTA È SOLO LA BASE</span><h2>Vuoi adattarlo davvero a come lavorate voi?</h2><p>Parti da questa configurazione, cambia funzioni e struttura e decidi soltanto alla fine se acquistarlo.</p></div><a id="cta" href="${studio}">Personalizza questa versione →</a></section><div class="meta"><span>Demo Easy Come · dati operativi dimostrativi</span><span class="google" translate="no">Google Maps</span></div></main></div>`;
