(function(){
  ...
  function topbar(title,subtitle){
    const {place}=payload;
    return `<header class="view-top"><div><span class="crumb">${esc(payload.templateLabel||'HOSPITALITY')} / ${esc(title.toUpperCase())}</span><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div><div class="view-user"><span>${esc(initials(place.name))}</span><div><strong>${esc(place.name)}</strong><small>Ambiente dimostrativo</small></div></div></header>`;
  }
  ...
  function dashboardView(){
    ...
    return `${topbar('Panoramica',`Oggi · ${place.category}${place.address?' · '+place.address:''}`)}
      <section class="hero"><div><span>CENTRO OPERATIVO</span><h2>Booking, soggiorni e numeri nello stesso sistema.</h2><p>Una simulazione Easy Come Hospitality costruita per <b>${esc(place.name)}</b>. Tutti i clienti, importi e operazioni mostrati qui sono fittizi.</p></div><button data-section-jump="1">Apri area operativa →</button></section>
  ...
  function listPreset(label){
    ...
    if(s.includes('sito')||s.includes('booking'))return {title:'Booking Connector',sub:'Prenotazione diretta collegata al sito esistente e al gestionale',heads:['Canale','Funzione','Risultato','Stato'],rows:[['Sito diretto','Disponibilità live','Prenotazione nel PMS','Attivo'],['Pagamento','Caparra online','Incasso collegato','Attivo'],['Conferma','Email ospite','Automatico','Pronto'],['Calendario','Stesso inventario','Meno passaggi','Connesso']]};
    if(s.includes('camere'))return {title:'Camere & alloggi',sub:'Disponibilità, capienza e stato operativo',heads:['Alloggio','Tipologia','Stato','Pulizia'],rows:[['Matrimoniale 1','Matrimoniale','Occupata','Pronta'],['Matrimoniale 2','Matrimoniale','Arrivo oggi','Pronta'],['Camera Deluxe','Deluxe','Libera','Pronta'],['Family 1','Family','Partenza oggi','Da pulire']]};
    if(s.includes('pulizie'))return {title:'Pulizie',sub:'Cosa preparare e per quale arrivo',heads:['Alloggio','Quando','Prossimo ospite','Stato'],rows:[['Family 1','Oggi 11:00','Anna Klein','Da fare'],['Camera Deluxe','Oggi 12:00','Giulia Romano','In corso'],['Matrimoniale 2','Domani','Marco Riva','Pianificata'],['Matrimoniale 1','—','—','Pronta']]};
  ...
  async function init(){
    try{
      ...
      $('#app').className='';
      const quotedPrice=Number(d.price||99);const startingPrice=Number(d.startingPrice||99);
      $('#app').innerHTML=`<div class="demo-shell"><aside class="side"><div class="brand"><span class="brand-mark">EC</span><div><small>ANTEPRIMA CREATA PER</small><strong>${esc(place.name)}</strong></div></div><div class="demo-badge">DEMO INTERATTIVA</div><div class="side-price"><small>QUESTA CONFIGURAZIONE</small><strong>${money(quotedPrice)}</strong><span>una tantum</span></div><nav class="nav">${model.nav.map((x,i)=>`<button class="${i===activeIndex?'active':''}" data-index="${i}"><span>${sectionIcon(x)}</span><b>${esc(x)}</b></button>`).join('')}</nav><div class="side-info"><span>Questa è una simulazione</span><p>I dati operativi sono inventati. Il nome e le informazioni pubbliche dell’attività servono solo a rendere l’anteprima concreta.</p></div><div class="side-footer"><b>Easy Come</b><small>Dal sito alla struttura.</small></div></aside><main class="main"><div class="demo-topline"><div><span class="pulse"></span> ${esc(expiry)}</div><div class="demo-quote"><span>Easy Come Hospitality da ${money(startingPrice)}</span><strong>Questa demo: ${money(quotedPrice)}</strong></div><div class="top-actions"><a href="${studio}" id="topCta">Adattalo alla tua struttura</a></div></div><div id="view"></div><section class="cta"><div><span>PREZZO INDICATIVO · ${money(quotedPrice)} UNA TANTUM</span><h2>Vuoi adattarlo davvero alla tua struttura?</h2><p>Questa configurazione è già prezzata in base alle funzioni mostrate. Puoi modificarla e vedere il prezzo aggiornarsi prima di acquistare. Easy Come Hospitality parte da ${money(startingPrice)} e il totale cambia in base alle funzioni scelte.</p></div><a id="cta" href="${studio}">Personalizza questa versione →</a></section><div class="meta"><span>Demo Easy Come · dati operativi dimostrativi · prezzo indicativo</span><span class="google" translate="no">Google Maps</span></div></main></div>`;
