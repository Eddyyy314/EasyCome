...
import { buildWebsiteProfile } from '../_website-factory.js';

function cleanPlace(p){return{name:p.displayName?.text||'La tua attività',address:p.formattedAddress||'',website:p.websiteUri||'',phone:p.nationalPhoneNumber||'',category:p.primaryTypeDisplayName?.text||p.primaryType||'Attività',primaryType:p.primaryType||'',placeId:p.id||'',types:p.types||[]}}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const websiteProfile=target.demo_config?.websiteProfile||buildWebsiteProfile(place,target.template_id,{});
    res.setHeader('cache-control','private, no-store, max-age=0');
    return res.status(200).json({slug,place,model,project,price,startingPrice:198,expiresAt:target.expires_at,views:count,templateLabel:t.label,websiteProfile,googleMapsAttribution:true});
