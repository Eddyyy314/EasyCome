...
export default async function handler(req,res){
  ...
  try{
    ...
    const snap=cfg.placeSnapshot||{};return json(res,200,{ok:true,proposal:{companyName:snap.name||p.companyName||'La tua attività',category:snap.category||'',address:snap.address||'',previewUrl:p.hostedPreviewUrl||p.previewUrl||'',previewMode:p.previewMode||'',siteFileCount:Number(p.siteFileCount||0),price:Number(p.price||0),implementationFee:Number(p.implementationFee??50),totalPrice:Number(p.totalPrice??(Number(p.price||0)+50)),currency:'EUR',status:p.status||'ready',paidAt:p.paidAt||null,packageReady:Boolean(p.packagePath),createdAt:p.createdAt||null,expiresAt:p.expiresAt||null}});
