...
import { createOrder, updateOrderById, getSubscriptionByUser, getProjectByUserId } from '../_supabase.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    const projectRow=await getProjectByUserId(user.id);
    if(!projectRow) throw new Error('Easy Come Audit può essere attivato solo se possiedi un Gestionale Easy Come collegato al tuo account.');
    const existing=await getSubscriptionByUser(user.id);
    if(existing&&!['canceled','incomplete_expired'].includes(existing.status)) throw new Error('Easy Come Audit è già attivo sul tuo account. Apri “Gestisci abbonamento” dal profilo.');
    ...
    const amountCents=Number(process.env.EASYCOME_AUDIT_MONTHLY_CENTS||10000);
    if(!Number.isInteger(amountCents)||amountCents<100) throw new Error('Prezzo Easy Come Audit non valido.');
    await createOrder({
      id:orderId,user_id:user.id,status:'checkout_created',customer_email:email,customer_name:customerName,
      company_name:companyName,amount_cents:amountCents,currency:'eur',price_breakdown:{auditMonthly:amountCents/100,managedMonthly:amountCents/100},
      project:{company:{name:companyName,email},delivery:{auditServiceSelected:true,managedServiceSelected:true}},delivery_status:'not_ready',download_count:0,
    ...
    add('metadata[order_id]',orderId);add('metadata[user_id]',user.id);add('metadata[purchase_type]','audit_service');add('metadata[audit_service]','selected');
    add('subscription_data[metadata][order_id]',orderId);add('subscription_data[metadata][user_id]',user.id);add('subscription_data[metadata][plan_code]','audit_100');
