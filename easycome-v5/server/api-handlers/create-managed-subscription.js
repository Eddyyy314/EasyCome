...
export default async function handler(req,res){
  ...
  try{
    ...
    const amountCents=Number(process.env.EASYCOME_MANAGED_MONTHLY_CENTS||3000);
    ...
    await createOrder({
      ...
      purchase_type:'managed_service',managed_service_selected:true,
    });
    ...
    add('line_items[0][price_data][recurring][interval]','month');add('line_items[0][price_data][product_data][name]','Gestione tecnica Easy Come');
    ...
    add('metadata[order_id]',orderId);add('metadata[user_id]',user.id);add('metadata[purchase_type]','managed_service');
    add('subscription_data[metadata][order_id]',orderId);add('subscription_data[metadata][user_id]',user.id);add('subscription_data[metadata][plan_code]','managed_tech_30');
