...
export default async function handler(req, res) {
  ...
  try {
    ...
    const legalAcceptance = {
      ...
      terms_version: clean(legal.termsVersion || 'EC-TOS-2026-08-29-v3', 80),
      refund_policy_version: clean(legal.refundPolicyVersion || 'EC-REF-2026-08-29-v3', 80),
      client_accepted_at: clean(legal.acceptedAt, 80) || null,
      server_recorded_at: new Date().toISOString(), user_id: user.id, email,
    };
    ...
    await createOrder({
      ...
      source_url: clean(body.sourceUrl, 700) || null, prepared_filename: clean(body.preparedFilename, 250) || null,
      delivery_status: 'not_ready', download_count: 0, purchase_type: 'software_package', managed_service_selected: false,
    ...
    add('line_items[0][price_data][product_data][description]', 'Easy Come Zero-touch: Stay Readiness, Exception Inbox, Stay Pack e regole operative configurate per la struttura.');
    ...
    add('line_items[1][price_data][product_data][description]', 'Mappatura del processo, configurazione iniziale, regole operative e avvio del sistema nel perimetro dell’ordine.');
    add('metadata[order_id]', orderId);
    add('metadata[company_name]', companyName.slice(0, 480));
    add('metadata[price_version]', 'easycome-v14-zero-touch-2026-09');
