...
import { buildWebsiteProfile, buildWebsiteFiles } from './_website-factory.js';
const global = { EasyZip: { createZipBytes }, ECProductTemplates, crypto: globalThis.crypto };
  ...
  const MODULES = [
    ...
    { id: 'website', name: 'Sito web Easy Come', category: 'Canali', price: 24, description: 'Sito multipagina responsive, SEO locale, richieste collegate al gestionale, privacy, sitemap e pacchetto pronto alla pubblicazione.', entities: ['public_submissions'] },
...
`;
  ...
  function websiteTemplateForProject(project) {
    const mods=new Set(project.modules||[]); const raw=String(project.templateId||'').trim();
    if(raw)return raw;
    if(mods.has('bookings'))return 'booking';
    if(mods.has('appointments'))return 'appointments';
    if(mods.has('projects')||mods.has('assets'))return 'projects';
    if(mods.has('inventory')||mods.has('orders'))return 'retail';
    return 'professional';
  }
  function generatedWebsitePackage(project){
    const profile=buildWebsiteProfile({name:project.company.name,category:project.company.industry,address:project.company.address||'',phone:project.company.phone||'',email:project.company.email||''},websiteTemplateForProject(project),project.website||{});
    return buildWebsiteFiles(profile,{organizationId:project.organizationId||'',supabaseUrl:project.identity?.supabaseUrl||'',supabaseAnonKey:project.identity?.supabaseAnonKey||'',siteUrl:project.website?.siteUrl||'https://example.com'}).map(file=>({name:`public-site/${file.name}`,data:file.data}));
  }
  ...
  function generatePackage(projectInput) {
    ...
    const files = [
      ...
      ...((project.modules || []).includes('website') ? generatedWebsitePackage(project) : []),
