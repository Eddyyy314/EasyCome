...
export function buildProject(place,_templateId='hospitality',ownerEmail=''){
  ...
  p.delivery.packagePrice=99;p.delivery.implementationSelected=true;p.delivery.implementationPrice=150;p.delivery.managedServiceSelected=false;p.delivery.managedServicePrice=0;p.delivery.previewApproved=true;
...
export function demoPrice(place){return Number(ECGenerator.calculatePrice(buildProject(place)).total||249)}
export function outreachSubject(place){return `Abbiamo preparato Easy Come per ${place.displayName?.text||'la vostra struttura'}`}
export function outreachMessage(place,demoUrl,price=249){const name=place.displayName?.text||'la vostra struttura';return `Buongiorno,\n\nsono Edoardo di Easy Come. Ho preparato una demo operativa per ${name}.\n\nNon è un altro calendario/PMS: Easy Come controlla che ogni soggiorno sia davvero pronto — ospite, soldi, pratica, casa, accesso e follow-up — e porta al titolare solo le eccezioni che richiedono una decisione.\n\n${demoUrl}\n\nL'offerta di lancio è €${price} una tantum e include la mappatura iniziale del vostro processo. Potete provare la demo senza acquistare nulla.\n\nUn saluto,\nEdoardo\nEasy Come`; }
