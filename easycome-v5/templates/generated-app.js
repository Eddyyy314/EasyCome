...
(() => {
  ...
  const ownerUserId = project.identity?.ownerUserId || '';
  const ownerEmail = String(project.identity?.ownerEmail || company.email || '').toLowerCase();
  ...
  function availableModes(entity) {
    const modes = [{ id: 'table', label: 'Tabella' }, { id: 'sheet', label: 'Griglia avanzata' }];
  ...
  function sidebar() {
    ...
    return `<aside class="sidebar">
      <div class="brand"><div class="brand-mark">${brandMark()}</div><div class="brand-copy"><strong>${esc(company.name || 'Gestionale')}</strong><span>${authReady ? 'Easy Come ID' : 'Demo interattiva'}</span></div></div>
      <div class="nav-label">Panoramica</div>
      <nav class="nav"><button data-view="dashboard" class="${state.view === 'dashboard' ? 'active' : ''}"><span class="nav-icon">⌂</span>Panoramica</button></nav>
      ...
      <div class="nav-label">Easy Come</div>
      <nav class="nav">
        <button data-hub><span class="nav-icon">EC</span>Easy Come Hub</button>
        <button data-manual><span class="nav-icon">?</span>Manuale</button>
        <button data-view="settings" class="${state.view === 'settings' ? 'active' : ''}"><span class="nav-icon">⚙</span>Impostazioni</button>
      </nav>
      <div class="sidebar-footer"><strong>${esc(state.role.toUpperCase())}</strong><br>${cloudReady ? 'Dati cloud protetti da Supabase RLS.' : authReady ? 'Accesso Easy Come attivo · dati del pacchetto in modalità locale.' : 'I dati della demo restano in questo browser.'}</div>
    </aside>`;
  }
  ...
  function shell() {
    ...
    $('[data-hub]')?.addEventListener('click', () => window.open('easycome-hub.html', '_blank', 'noopener'));
    $('[data-manual]')?.addEventListener('click', () => window.open('manuale.html', '_blank', 'noopener'));
  ...
  async function renderDashboard() {
    ...
    try {
      ...
      main.innerHTML = `<header class="topbar"><div><h1>Panoramica</h1><p>${esc(today.charAt(0).toUpperCase() + today.slice(1))} · dati aggiornati</p></div><div class="top-actions"><span class="sync-pill ${cloudReady ? 'online' : 'demo'}">${cloudReady ? '● Cloud attivo' : '● Demo locale'}</span><button class="btn btn-secondary" id="quickBackup">Backup</button><div class="avatar">${esc(initials())}</div></div></header>
        ...
        <section class="insight-grid">${lowStock.length ? `<article class="insight danger"><span>SCORTE DA CONTROLLARE</span><strong>${lowStock.length} prodotti sotto soglia</strong><p>${lowStock.slice(0, 4).map(({ row }) => esc(row.name || row.sku || 'Prodotto')).join(' · ')}</p><button data-open-section="products">Apri magazzino</button></article>` : ''}<article class="insight"><span>EASY COME HUB</span><strong>Manuale e assistenza sempre disponibili</strong><p>Segnala problemi, chiedi nuove funzioni e segui l’avvio del sistema.</p><button data-open-hub>Apri Easy Come Hub</button></article></section>
      ...
      $('[data-open-hub]')?.addEventListener('click', () => window.open('easycome-hub.html', '_blank', 'noopener'));
  ...
  function renderSettings() {
    ...
    main.innerHTML = `<header class="topbar"><div><h1>Impostazioni</h1><p>Controllo, sicurezza e portabilità dei dati</p></div><div class="top-actions"><div class="avatar">${esc(initials())}</div></div></header>
      ...
      <article class="card settings-card"><div class="card-head"><div><h2>Account</h2><p>${cloudReady ? esc(state.session?.user?.email || '') : 'Sessione dimostrativa'}</p></div></div><div class="card-body button-stack"><button class="btn btn-secondary" id="openHubSettings">Apri Easy Come Hub</button><button class="btn btn-secondary" id="openManualSettings">Apri manuale</button>${authReady ? '<button class="btn btn-danger" id="logout">Esci dal gestionale</button>' : '<button class="btn btn-secondary" id="resetDemo">Ripristina dati demo</button>'}</div></article>${modules.has('multiuser') && cloudReady ? '<article class="card settings-card" id="membersCard"><div class="card-body"><div class="empty compact">Caricamento collaboratori…</div></div></article>' : ''}</section>
      <section class="card setup-card"><div><span>PRIMA DELLA CONSEGNA</span><h2>Checklist operativa</h2><p>Verifica l’accesso Easy Come, prova i ruoli, completa l’onboarding nel Hub, controlla le automazioni e scarica un backup iniziale.</p></div><ol><li>Login del titolare verificato</li><li>Permessi collaboratori testati</li><li>Easy Come Hub e manuale verificati</li><li>Backup iniziale consegnato</li></ol></section>`;
    ...
    $('#openHubSettings')?.addEventListener('click', () => window.open('easycome-hub.html', '_blank', 'noopener'));
    $('#openManualSettings')?.addEventListener('click', () => window.open('manuale.html', '_blank', 'noopener'));
  ...
  function auth(message = '') {
    app.innerHTML = `<section class="auth"><div class="auth-hero"><div class="brand-mark">${brandMark()}</div><span class="auth-kicker">ACCESSO CON EASY COME ID</span><h1>${esc(company.name || 'Il tuo gestionale')}</h1><p>${esc(company.description || 'Tutto il lavoro della tua azienda in un unico spazio semplice, veloce e protetto.')}</p><div class="auth-features"><span>✓ Stesse credenziali di Easy Come</span><span>✓ Manuale e supporto nello stesso account</span><span>✓ Dati sempre esportabili</span></div></div><div class="auth-panel"><div class="auth-card"><span class="auth-logo">${esc(initials())}</span><h2>Accedi al gestionale</h2><p>${authReady ? 'Usa l’email e la password con cui ti sei registrato su Easy Come.' : 'La configurazione Easy Come ID non è ancora collegata: puoi aprire la demo locale.'}</p>${message ? `<div class="alert alert-error">${esc(message)}</div>` : ''}<form id="authForm"><label>Email<input type="email" name="email" autocomplete="email" required value="${esc(project.identity?.ownerEmail || company.email || '')}"></label><label>Password<input type="password" name="password" autocomplete="current-password" minlength="8" required></label><button class="btn btn-primary" type="submit">Accedi con Easy Come</button></form><div class="auth-links">${authReady ? '<button id="forgotPassword" type="button">Password dimenticata?</button>' : ''}<a href="${esc(easycomeBaseUrl)}" target="_blank" rel="noopener">Gestisci il tuo account su Easy Come</a></div>${!authReady ? '<div class="demo-box"><strong>Modalità dimostrativa</strong><p>Puoi provare tutte le viste senza collegare un database.</p><button class="btn btn-primary" id="demoEnter">Apri la demo</button></div>' : ''}</div></div></section>`;
    ...
    $('#authForm').onsubmit = async (event) => {
      ...
      try {
        if (!authReady) { state.session = { user: { email: credentials.email } }; state.role = 'owner'; shell(); return; }
        const response = await db.auth.signInWithPassword(credentials);
        if (response.error) throw response.error;
        state.session = response.data.session;
        if (!state.session) throw new Error('Accesso non completato.');
        if (!accountAllowed(state.session.user)) { await db.auth.signOut(); state.session = null; throw new Error('Questo gestionale appartiene a un altro account Easy Come.'); }
        if (cloudReady) {
          try { await db.rpc('claim_owner_by_email', { p_organization_id: orgId }); } catch (_) {}
          await fetchRole();
          if (!can('read')) throw new Error('Questo account non è ancora autorizzato per il gestionale.');
        } else state.role = 'owner';
        shell();
      } catch (error) {
        auth(error.message || String(error));
      }
    };
  }
  ...
  async function init() {
    if (authReady) {
      ...
      if (state.session) {
        if (!accountAllowed(state.session.user)) { await db.auth.signOut(); state.session = null; auth('Questo gestionale appartiene a un altro account Easy Come.'); return; }
        if (cloudReady) {
          try { await db.rpc('claim_owner_by_email', { p_organization_id: orgId }); } catch (_) {}
          try { await fetchRole(); if (can('read')) { shell(); return; } } catch (_) {}
        } else { state.role = 'owner'; shell(); return; }
      }
    }
    auth();
  }
